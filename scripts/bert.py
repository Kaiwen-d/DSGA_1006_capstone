...
def get_summary(document):
    ...
    for i in document["document"]:
        ...
        if length >= 5:
            ...
            sub_summary = bert(cluster_string, num_sentences=3)
            summary.append([sub_summary])

        else:
            cluster_string = "".join(i)
            sub_summary = bert(cluster_string, num_sentences=1)
            summary.append([sub_summary])
...
result.save_to_disk("/scratch/sl6246/DSGA_1006_capstone/dataset/multi_news_test_bert/shard_"+ sys.argv[1])